...
class LigneController extends Controller
{
    ...
    {
        return LigneResource::collection(Ligne::query()->orderBy('id', 'desc')->paginate(10));
    }
    ...
    //recuprer le date de reservation full 
    ...
    //
    public function getdateall($voyage_id)
    {
        $disableddate= Reservation::where('voyage_id',$voyage_id)->get('voyagedate');
        return response()->json($disableddate);
    }
    //recuprer les reservations par le voyage et date 
...
//ajouter une billets a la base de données
...
//admin application peut voir toutes les entreprise 
public function getentreall()
    ...
    //recuprer les  données d'une billet 
 public function getbillet($numero)
    ...
    //recuprer une voyage par ses id 
    ...
    //recupree les lignes par ses id 
...
// gett all reservations par le voyagte id 
public function getrall($id)
    {
    $reserve = Reservation::where('voyage_id',$id)->get();
    return response()->json($reserve);
  }
  //aprés le selcetion d'une date et voyage selecté une resetrvation 
  ...
  //function change le etat de billet si le date est passé
